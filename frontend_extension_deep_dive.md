...
4.  **`getMangaUpdate(...)`**: Retrieves detailed metadata from `/manga/details` using the manga's URL. If `fetchDetails` is enabled, it maps the synopsis and genres. If `fetchChapters` is enabled, it parses the chapters list, ordering them from newest to oldest (Index 0 = Newest) using the backend-supplied `chapter_number` float value directly without `.reversed()` transformations. It maps each chapter using `url = chapterItem.url` to store the absolute URL in the local SQLite database instead of stripping the domain with `setUrlWithoutDomain(chapterItem.url)` (which previously caused routing conflicts by resolving relative paths to the default meshmanga.com base domain).
...
3.  **[COMPLETED] Chapter URL Absolute Mapping (Hotfix 2)**: Modified `NeoMangaMasterExtension.kt` inside `getMangaUpdate` to assign `url = chapterItem.url` instead of `setUrlWithoutDomain(chapterItem.url)`. This preserves the absolute URL (and domain) of each chapter (either meshmanga.com or olympustaff.com) in the local SQLite database, avoiding relative path resolving conflicts with the default `siteUrl`.
